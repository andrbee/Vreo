...
$Path = get_post_meta($postID, 'amazon-path');
...
$rolePath .= "/";
$Path = get_post_meta($postID, 'amazon-path', true);
use Aws\S3\S3Client;
...
if (!empty($sliderCampaign && file_exists($pathSliderServer))) {
    ...
    if (is_array(get_post_meta($postID, "campaignSlider"))) {
        foreach (get_post_meta($postID, "campaignSlider") as $s) {
            $campaignSlider = array_merge($campaignSlider, $s);
        }

    } else {
        array_unshift($campaignSlider, get_post_meta($postID, "campaignSlider"));
//        exit;
    }
//    print_r($campaignSlider);
//    exit;
...
foreach ($postList as $post):
    if (empty($campaign_video) && empty($campaign_video_key)) {
        $campaign_video = $post->campaign_video;
        $campaign_video_key = $post->campaign_video_key;
    }
    if (empty($campaign_bg_header) && empty($campaign_bg_header_file)) {
        $campaign_bg_header = $post->campaign_bg_header;
        $campaign_bg_header_file = $post->campaign_bg_header_file;
    }
    if (empty($campaign_age) && empty($campaign_age_file)) {
        $campaign_age = $post->campaign_age;
        $campaign_age_file = $post->campaign_age_file;
    }
    if (empty($campaign_image) && empty($campaign_image_file)) {
        $campaign_image = $post->campaign_image;
        $campaign_image_file = $post->campaign_image_file;
    }
    if (empty($campaign_bg) && empty($campaign_bg_file)) {
        $campaign_bg = $post->campaign_bg;
        $campaign_bg_file = $post->campaign_bg_file;
    }
    if (empty($placeholder_plugin) && empty($placeholder_plugin_file)) {
        $placeholder_plugin = $post->placeholder_plugin;
        $placeholder_plugin_file = $post->placeholder_plugin_file;
    }

    switch ($_POST['active']) {
        case 'publish': {
            if (empty($campaignSlider)) {
                $campaignSlider=get_post_meta($postID,'campaignSlider')[0];
                $post_data = array(
                    'ID' => $postID,
