...
switch ($_GET['formSet']) {
    case 'message':
        ...
        if ($result2[0]->id_brand == $cur_user_id) {
            ...
            $serverUrls = "/update-third-step/?idDev={$idDevPlace}&pageId={$postIdPlace}";
        ...
        header("location: /update-third-step/?idDev={$idDevPlace}&pageId={$postIdPlace}");
    ...
    case 'delete':
        ...
        $resultID = $wpdb->get_results("SELECT * FROM pl_step_files_update WHERE id = $idFile");

        $del = unlink(__DIR__ . '/../../../../uploads/' . $user_brand->user_login . '/' . $resultID[0]->path_file);

        if ($del) {
            $wpdb->delete('pl_step_files_update',
    ...
    case 'approve':
        ...
        $resultID = $wpdb->get_results("SELECT * FROM pl_step_files_update WHERE id = $idFile id_post = $postIdPlace");

        if ($role == 'brand') {
            $wpdb->update('pl_step_files_update',
    ...
    case 'finish':
        ...
        if ($result2[0]->id_brand == $cur_user_id) {
            if (!$statusBrand) {
                ...
                $statusBrand = $wpdb->get_results("SELECT * FROM ps_finish_update WHERE postIdPlace = $postIdPlace AND idDevPlace = $idDevPlace AND status = 1 AND curIdPlace = $id_brand ORDER BY id DESC LIMIT 1");

                $statusDev = $wpdb->get_results("SELECT * FROM ps_finish_update WHERE postIdPlace = $postIdPlace AND idDevPlace = $idDevPlace AND status = 1 AND curIdPlace = $id_dev ORDER BY id DESC LIMIT 1");

                if ($statusBrand && $statusDev) {
                    $resultStep = $wpdb->get_results("SELECT * FROM pl_step_files_update WHERE id_dev = $idDevPlace AND id_post = $postIdPlace AND approve = 1 AND approve_brand = 1");
                    $wpdb->insert(
                        'activities',
                        array(
                            'id_user' => $id_dev,
                    ...
                    $resultGetPlugin = $wpdb->get_results("SELECT * FROM pl_get_random_advertiser WHERE developer_game_id = $postIdPlace ");
                    $argsGetPlugin = [];
                    foreach ($resultGetPlugin as $key) {
                        $argsGetPlugin[] = $key->id;
                    }
                    foreach ($resultStep as $key => $value) {
                        ...
                        $wpdb->update(
                            ...
                            array('id' => $argsGetPlugin[$key])
                        );
                    }
                    if (count($resultStep) != count($argsGetPlugin)) {
                        for ($i = count($resultStep); $i < count($argsGetPlugin); $i++){
                            $wpdb->update(
                                'pl_get_random_advertiser',
                                array(
                                    'type_media_id' => 1,
                    ...
                    foreach ($resultStep as $item => $value) {
                        $wpdb->delete( 'pl_step_files_update', array( 'id' => $resultStep[$item]->id ) );
                    }
                    $resultFinish = $wpdb->get_results("SELECT * FROM ps_finish_update WHERE postIdPlace = $postIdPlace AND status = 1");
                    foreach ($resultFinish as $item) {
                        $wpdb->delete( 'ps_finish_update', array( 'id' => $item->id ) );
                    }
                    $post_data = array(
                        'ID' => $postIdPlace,
