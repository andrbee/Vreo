	...
	<div class="page-content-wrapper">
		...
		<div class="page-content">
			...
			<div class="breadcrumbs page-bar" typeof="BreadcrumbList" vocab="https://schema.org/">
				...
				<a href="/place-step-first/?idDev=<?=$post->post_author;?>&pageId=<?=$post->ID;?>" class="btn btn-primary marketplace-btn" target="_blank"><span class="glyphicon glyphicon-flag"> </span> Place your brand</a>
				<?php endif; ?>
			</div>
			<!-- END PAGE BAR -->
			<!-- END PAGE HEADER-->
		...
		while ( have_posts() ) : the_post();
			...
			$categs=(array)get_the_category(); // получаем все категории поста
			$cat_ids=array();// инит. массив будущих ID-шников
			foreach ($categs as $cat_id) {
				$cat_ids[]=$cat_id->cat_ID; // записываем все id в массив
			}
//			if(in_array(8,$cat_ids)) // проверяем если в массиве ID-шников ID=8 - 'Marketplace'
			if(true) // проверяем если в массиве ID-шников ID=8 - 'Marketplace'
			{
				get_template_part( 'template-parts/content', 'campaign' ); // Открываем шаблон Marketplace
			} else {
