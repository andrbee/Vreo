...
<div class="row">
    ...
    <?php endif; ?>
        <br>
        <form action="<?php echo get_stylesheet_directory_uri() . '/inc/place-step/thirdStep.php?formSet=finish'; ?>"
              method="post">
...
<div class="table-scrollable">
    <?php
    $resultStep = $wpdb->get_results("SELECT * FROM pl_step_files WHERE id_dev = $idDevPlace AND id_post = $postIdPlace");
    $resultRole = $wpdb->get_results("SELECT * FROM pl_step_files WHERE id_dev = $idDevPlace AND id_post = $postIdPlace ORDER BY id DESC");
...
//            $amazonFile = $s3->getObject()
            ...
            <td>
                ...
                <form
                    action="<?php echo get_stylesheet_directory_uri() . '/inc/place-step/thirdStep.php?formSet=approve'; ?>"
                ...
                <form
                    action="<?php echo get_stylesheet_directory_uri() . '/inc/place-step/thirdStep.php?formSet=decline'; ?>"
                    style="display: inline-block" method="post">
                    <input type="hidden" name="idDevPlace" value="<?= $idDevPlace; ?>">
                    <input type="hidden" name="postIdPlace" value="<?= $postIdPlace; ?>">
                    <input type="hidden" name="curIdPlace" value="<?= $cur_user_id; ?>">
...
<?php if (!$status): ?>
    <div class="row">
        ...
        <div class="form-group ">
            ...
            <form
                action="<?php echo get_stylesheet_directory_uri() . '/inc/place-brand.php?idDev=' . $idDevPlace . '&pageId=' . $postIdPlace; ?>"
