...
$activitic_db = $wpdb->get_results( "SELECT * FROM activities WHERE id_user = $cur_user_id AND (data>=$dateDayDbStart and  data<$dateDayDbEnd) AND (categories IN(".implode(',',(array)$user_info->profile_categore).") OR hash_tag REGEXP '" . implode('|',(array)$user_info->profile_hash)."')");
$activitic_users_brand_db = $wpdb->get_results( "SELECT * FROM activities WHERE type_notifications = 'followUser' AND id_user = '$cur_user_id' AND (data>=$dateDayDbStart and  data<$dateDayDbEnd)");
$activitic_signed_brand_db = $wpdb->get_results( "SELECT * FROM activities WHERE type_notifications = 'signedUser' AND id_user = '$cur_user_id' AND (data>=$dateDayDbStart and  data<$dateDayDbEnd)");
$activitic_message_brand_db = $wpdb->get_results( "SELECT * FROM activities WHERE type_notifications = 'messages' AND id_user = '$cur_user_id' AND (data>=$dateDayDbStart and  data<$dateDayDbEnd)");
...
$allResultBrand = array_merge($activitic_message_brand_db,$activitic_signed_brand_db,$activitic_users_brand_db, $activitic_db,$activitic_profile_db,$activitic_admin_db,$step_message_db,$step_price_db,$step_approve_db);
...
uasort($allResultBrand,"dbSort");
$resultBrand = array_reverse($allResultBrand);
?>
<?
if(!empty($allResultBrand)){?>
		...
		<div class="tab-content">
			<div class="tab-pane active" id="tab_1_1">
				<div class="scroller" style="" data-always-visible="1" data-rail-visible="0">
					<ul class="feeds">
						<?php foreach($resultBrand as $key): ?>
							<li>
								<div class="col1">
									<div class="cont">
										<div class="cont-col1">
